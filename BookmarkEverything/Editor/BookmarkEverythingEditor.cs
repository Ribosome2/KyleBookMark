...
namespace BookmarkEverything
{
	public class BookmarkEverythingEditor : EditorWindow
    {
        ...
        public class EntryData
        {
            ...
            public int Index;
            /// <summary>
            /// Save this file path because GUID to path not always work ,like we deleted an asset or we switch VCS branch,
            /// We still need to show file path and keep them 
            /// </summary>
            public string FilePath;  
            public EntryData(string guid, string category, int index,string filePath)
            {
                GUID = guid;
                Category = category;
                Index = index;
            ...
            {
                return new EntryData(data.GUID, data.Category, data.Index,data.FilePath);
            }
            public static EntryData[] Clone(EntryData[] data)
            {
        ...
        [MenuItem("Window/Bookmark Everything %J")]
        ...
        public static List<EntryData> GetPrefPathList()
        {
            List<EntryData> resultList=new List<EntryData>();
            var prefSettings = IOHelper.ReadFromDisk<SaveData>(SETTINGS_FILENAME);
            //if nothing is saved, retrieve the default values
            if (prefSettings != null)
            {
                resultList.AddRange(EntryData.Clone(prefSettings.EntryData.ToArray()));
            }
        ...
        private void CheckInput()
        {
            if (Event.current != null)
            {
                ...
                if (Event.current.type == EventType.KeyUp)
                {
                    ...
                    {
                        HandleTabSwitch();
                        HandleSelectContent(Event.current.keyCode);
                    }
...
#region MAIN HEADER DRAWERS
        ...
        private int _curContentIndex = 0;
        ...
        void HandleSelectContent(KeyCode keyCode)
        {
            if (keyCode == KeyCode.UpArrow )
            {
                _curContentIndex--;
                _curContentIndex=Mathf.Clamp(_curContentIndex, 0, GetCurCategoryCount() - 1);
                Repaint();
        ...
        void HandleTabSwitch()
        {
            if (Event.current.type == EventType.KeyUp)
            {
                ...
                if (keyCode == KeyCode.RightArrow || keyCode == KeyCode.Tab)
                {
                    SwitchFinderTab(true);
                }
                else if(keyCode==KeyCode.LeftArrow)
                {
                    SwitchFinderTab(false);
                }
            }
        }

        void SwitchFinderTab(bool isToNext)
        {
            if (isToNext)
            {
                _projectFinderTabIndex++;
            }
            else
            {
                _projectFinderTabIndex--;
        ...
        private void ClampTabIndex()
        {
            if (_projectFinderTabIndex >= _projectFinderContents.Count)
            {
                _projectFinderTabIndex = 0;
            }
            else if (_projectFinderTabIndex <= -1)
            {
                _projectFinderTabIndex = _projectFinderContents.Count;
            }
        }

        int GetCurCategoryCount()
        {
            int count = 0;
            var category=_projectFinderHeaders[_projectFinderTabIndex];

            for (int i = 0; i < _currentSettings.EntryData.Count; i++)
            {
                var entryData = _currentSettings.EntryData[i];
        ...
        void TryPingCurSelection()
        {
            ...
            for (int i = 0; i < _currentSettings.EntryData.Count; i++)
            {
                ...
                if (entryData.Category == category)
                {
                    if (index == _curContentIndex)
                    {
                        string path = AssetDatabase.GUIDToAssetPath(entryData.GUID);
                        bool exists = IOHelper.Exists(path);
                        TryPingByPath(exists, path);
        ...
        private void DrawProjectFinder()
        {
            ...
            var category=_projectFinderHeaders[_projectFinderTabIndex];
            DrawProjectFinderEntries(category);
...
//            }
        ...
        private void DrawProjectFinderEntries(string category)
        {
            ...
            for (int i = 0; i < _currentSettings.EntryData.Count; i++)
            {
                ...
                if (entryData.Category == category)
                {
                    ...
                    {
                        if (!string.IsNullOrEmpty(entryData.FilePath))
                        {
                            path = entryData.FilePath;
                        }
                        content = RetrieveGUIContent((isFolder ? GetNameForFolder(path) : GetNameForFile(path)) + "(File is removed)" , "console.erroricon.sml");
                    }
                    EditorGUILayout.BeginHorizontal();
                    clicked = DrawSingleEntryData(index, content, exists, path, clicked);
        ...
        private bool DrawSingleEntryData(int dataIndex, GUIContent content, bool exists, string path, bool clicked)
        {
            Color prev = GUI.color;
            GUI.color = dataIndex == _curContentIndex ? Color.cyan : prev;
            ...
            {
                clicked = TryPingByPath(exists, path);
        ...
        private bool TryPingByPath(bool exists, string path)
        {
            bool clicked=false;
            if (exists)
            {
